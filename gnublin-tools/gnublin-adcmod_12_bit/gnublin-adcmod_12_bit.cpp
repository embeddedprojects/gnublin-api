...
void pars_opts(int argc, char **argv) {
	...
	while((c = getopt(argc,argv,"hc:bja:d")) != -1){
			...
			case 'a' : address = strtol(optarg,NULL,16); break;
			case 'd' : dif_flag=1; break;
		}
	}

	if (hflag | (argc<=1)){
		printf("This Script prints out the 12-bit value of a selected adc-channel of the gnublin-adc-module_12_bit \n\n"
		"-h Show this help\n"
		"-b show raw output\n"
		...
		"gnublin-adcmod_12_bit -j -a 0x48 -c 2 \n\n"

		"channel=7, I2C-Address=0x4A print value in normal format:\n"
		"gnublin-adcmod_12_bit -a 0x4A -c 7\n\n");
...
int main(int argc, char **argv) {
	...
	if (dif_flag) {
		if (bare_flag){
			...
			if (adc.fail()) {
				error_msg((char*)"getting value differential read");
			}
		}
		else {
			value = adc.getVoltage(channel, channel+1);
			if (adc.fail()) {
				error_msg((char*)"getting value at differential read");
			}
		}
		...
		if (bare_flag){
			...
			if (adc.fail()) {
				error_msg((char*)"getting value at single channel read");
			}
		}
		else {
