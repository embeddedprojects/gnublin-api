...
void parse_opts(int argc, char **argv)
{
        ...
        while((c = getopt(argc,argv,"hjp:f:a:bi:d:x:n:o:")) != -1)
        {
                switch(c)
                {
                        ...
                        case 'p' : Motor.setMotorParam();
                        	Motor.getFullStatus1();
                        	Motor.runInit();
                        	Motor.drive(atoi(optarg));	         	break;
                        case 'a' : Motor.setAddress(strtol (optarg,NULL,16));	break;
                        case 'j' : json_flag = 1;                           	break;
                        case 'b' : brute_flag = 1;                          	break;
			case 'i' : Motor.setIrun(atoi(optarg));		    	break;
			case 'd' : Motor.setIhold(atoi(optarg));	    	break;
			case 'x' : Motor.setVmax(atoi(optarg));		    	break;
			case 'n' : Motor.setVmin(atoi(optarg));		    	break;
			case 'o' : Motor.burnNewAddress(strtol(optarg,NULL,16));break;
		}
