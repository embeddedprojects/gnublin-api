...
//*****************************************************************************
// Class for accesing GNUBLIN Module-ADC-12-BIT / ADS7828E
//*****************************************************************************
...
*/
gnublin_module_adc_12_bit::gnublin_module_adc_12_bit() {
	i2c.setAddress(0x48);
	referenceValue = 2500;
	reference_flag = IN;
	error_flag = false;
...
*/
const char *gnublin_module_adc_12_bit::getErrorMessage(){
...
*
* @~german
* @brief Gibt das errorflag zurück, um Fehler in der zuvor aufgerufenen Methode zu erkennen.
*
* @return error_flag
*/
bool gnublin_module_adc_12_bit::fail() {
	return error_flag;
}
...
*/
int gnublin_module_adc_12_bit::setAddress(int address) {
...
* @return 1 bei Erfolg, -1 im Fehlerfall
*/
int gnublin_module_adc_12_bit::setDevicefile(std::string filename) {
...
* @return 1 bei Erfolg, -1 im Fehlerfall
*/
int gnublin_module_adc_12_bit::setReference(int value) {
	if (value == 0) {
		referenceValue = 3300;
		reference_flag = 0;
	}
...
* @return 12-bit Wert
*/
int gnublin_module_adc_12_bit::getValue(int channel) {
	int command;
...
* @return Wert
*/
int gnublin_module_adc_12_bit::getValue(int channel1, int channel2) {
	int command = -1;
	unsigned char value[2];
	int val = 0;
...
* @return Wert in mV
*/
int gnublin_module_adc_12_bit::getVoltage(int channel) {
	error_flag = false;
	int voltage;
	...
	voltage = value * referenceValue / 4095;
	return voltage;
}
...
* @return Wert in mV
*/
int gnublin_module_adc_12_bit::getVoltage(int channel1, int channel2) {
	error_flag = false;
	int voltage;
